...
import org.cornutum.tcases.openapi.playwright.PlaywrightTestCaseWriter;
...
public class ApiTestCommand
{
  ...
  {
    public enum TestType { JUNIT, TESTNG, MOCO, PLAYWRIGHT };

    public enum ExecType { RESTASSURED, PLAYWRIGHT };
    ...
    public TestTarget createTestTarget()
    {
      ...
      switch (getExecType())
      {
        ...
        default:
        {
          testTarget = JavaTestTarget.builder()
                  .named( getTestName())
    ...
    public TestWriter<?,?> createTestWriter( TestCaseWriter testCaseWriter)
    {
      ...
      switch( getTestType())
      {
        ...
        case PLAYWRIGHT:
        {
          testWriter = new PlaywrightRunnerTestWriter( testCaseWriter);
          break;
        }
        default:
        {
    ...
    {
      BaseTestCaseWriter testCaseWriter;

      switch (getExecType())
      {
        case RESTASSURED:
        {
          testCaseWriter = new RestAssuredTestCaseWriter();
          break;
        }
        case PLAYWRIGHT:
        {
          testCaseWriter = new PlaywrightTestCaseWriter();
          break;
        }
        default: {
          throw new IllegalArgumentException( String.format( "%s is not a valid test type", getExecType()));
        }
      }
